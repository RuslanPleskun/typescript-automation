...
export class ShoppingCartPage {
  ...
  verifyFirstItemPresence(itemName: string) {
    this.getItem().first().should('contain.text', itemName);
  }

  verifySecondItemPresence(itemName: string) {
  ...
  verifyNumberOfItemsOnThePage(numberOfItems: number) {
    this.getItem().should('have.length', numberOfItems);
  }

  verifyItemAbsense(itemName: string) {
